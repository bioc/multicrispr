...
```{r}
cas9s <- score_cas9s(cas9s, bsgenome, method = 'Doench2014')
cas9s
```
...
```{r}
reticulate::use_condaenv('azi')
if (reticulate::py_module_available('azimuth')){
    cas9s <- score_cas9s(cas9s, bsgenome, method = 'Doench2016')
    cas9s
}
```

This allows us to discard Cas9 sites with a low expected targeting efficiency:
...
```{r, cache = TRUE}
cas9s2 <- filter_no_offtargets(cas9s, targets, bsgenome)
```
